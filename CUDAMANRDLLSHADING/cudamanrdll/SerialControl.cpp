...
SerialPortControl::SerialPortControl(unsigned int unUARTNo /*= 1*/, double dBauteRate /*=115200*/, unsigned int unDataBits /*= 8u*/, unsigned int unStopBits /*= 1u*/, char cParity /*= 'N'*/) {
	...
	//m_nTestLog = 0;
	m_bSerialPortInitSuccess = false;

	foutSerialPort.close();
	foutSerialPort.open("SerialPortLog.txt", ios_base::out);
}
...
int SerialPortControl::InitPort() {
	...
	foutSerialPort << "InitPort function run in:" << __LINE__ << endl;
	char pcComParam[50];
	m_bSerialPortInitSuccess = false;
...
bool SerialPortControl::GetSerialPortInitSuccess() {
	return m_bSerialPortInitSuccess;
}